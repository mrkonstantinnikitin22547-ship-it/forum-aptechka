        ...
        <div class="header-content">
            <nav class="main-nav">
                <div class="nav-container">
                    ...
                    <ul class="nav-list">
                        <li class="logo">
                            <img class="logo_img" src="{{ asset('img/logo-7.png') }}" alt="logo">
                            <div>
                                <h1>Аптечка</h1>
                                <p>Форум пациентов</p>
                            </div>
                        ...
                        <li>
                            <a href="{{ route('topics.index') }}" class="nav-link {{ request()->routeIs('topics.*') ? 'active' : '' }}">Темы</a>
                        </li>
                        <li>
                            <a href="{{ route('about') }}" class="nav-link {{ request()->routeIs('about') ? 'active' : '' }}">О форуме</a>
                        </li>
                        @auth

                        @if(auth()->user()->role === 'admin')
                        <li>
                            <a href="{{ route('admin.dashboard') }}"class="nav-link {{ request()->routeIs('admin.*') ? 'active' : '' }}"><i class="fas fa-tools"></i> Админ-панель </a>
                        </li>
                        @else
                        <li>
                            <a href="{{ route('profile') }}"class="nav-link {{ request()->routeIs('profile') ? 'active' : '' }}"><i class="fas fa-user-circle"></i> Профиль</a>
                        </li>
                        @endif
                        <li>
                        ...
                        @endauth
                        <li>
                            <a href="{{ route('support') }}"class="nav-link {{ request()->routeIs('support') ? 'active' : '' }}"><i class="fas fa-headset"></i> Техподдержка </a>
                        </li>
                    </ul>
                </div>
            </nav>
    ...
    <div class="modal-container">
        ...
        <div class="modal-body">
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="form-buttons">
                    <a href="{{ route('support') }}" class="btn btn-secondary btn-block">
